<template>
  <div style="width:950px">
    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template v-slot="scope">
          ...
          <el-button
              size="mini"
              @click="homeworkdDo(scope.$index, scope.row)">已完成</el-button>
          <el-button
              size="mini"
              @click="homeworkNotDo(scope.$index, scope.row)">未完成</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <div>
      <el-dialog title="未完成学生情况" v-model="dialogNotTableVisible">
        ...
        <el-pagination
            @size-change="handleSizeChange"
            @current-change="handleCurrentChange"
            :current-page="homework.page"
            :page-sizes="[10, 20, 30, 40]"
            :page-size="homework.pageSize"
            layout="total, sizes, prev, pager, next, jumper"
            :total="DoNotHomework.length">
        </el-pagination>
      </el-dialog>
    </div>
    <div>
      <el-dialog title="已完成学生情况" v-model="dialogTableVisible">
        <el-table :data="DoHomework">
          ...
          <el-table-column label="操作">
            <template v-slot="scope">
              <el-button
                  size="mini"
                  @click="DetailStudentHomework(scope.$index, scope.row)">查看详情</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
export default {
  ...
  created() {
    this.page.roleId=parseInt(Cookies.get('roleId'))
  ...
  methods:{
    DetailStudentHomework(index,row){
      ...
      this.$router.push({
        name:'Detail',
        query:{
          data2:JSON.stringify(row), // 将对象转为 JSON 字符串
          type:"作业",
          tag:'homework',
          check:true
        }
    ...
    homeworkNotDo(index,row){
      ...
      request.post('/study/user/findNotDoHomework',this.homework).then(resp=>{
        this.DoNotHomework=resp.data.data

      })
      this.dialogNotTableVisible=true
    },
    homeworkdDo(index,row){
      this.homework.homeworkId=row.id
      request.post('/study/userdohomework/list',this.homework).then(resp=>{
        console.log("homeworklist:",resp)
        this.DoHomework=resp.data.data
    ...
    handleDelete(index,row){
      ...
      request.post('/study/homework/delete',this.params).then(resp=>{
        if(resp.code=== '200'){
          this.$message({
            message: '恭喜你，删除成功',
            type: 'success'
    ...
    ToTest(){
      this.$router.push({
        ...
        query:{
          type:"作业",
          classId:parseInt(Cookies.get('classId')),
          tag:'homework'
        }
      })
    },
    handleSizeChange(size) {
      ...
      this.homeworkdDo(this.homework)
      this.thomework(this.page)
    ...
    thomework(page){
     request.post('/study/homework/list',page).then(resp=>{
